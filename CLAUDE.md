...
Before declaring any bug fix done, grep the affected file for every occurrence
of the pattern being fixed, not just the first one found. If the fix adds a
guard, exclusion, or condition to one call site, check whether the same pattern
exists elsewhere in the same file or module and apply the same treatment.
Never close a fix after patching only the first match.

## Cross-agent review

For non-trivial code or configuration changes, the agent performing the
implementation is the primary agent and owns the final result.

- When Codex is the primary agent, run `$claude-review` before declaring the
  implementation complete.
- When Claude Code is the primary agent, run `/codex:review --wait` before
  declaring the implementation complete.
- The reviewer is read-only and must not edit files, commit, push, or invoke
  another reviewer.
- Treat review findings as claims. The primary agent owns verification and
  final disposition. Codex should fix valid findings and explain rejections;
  Claude Code must preserve the Codex plugin's approval requirement before
  applying review fixes.
- After accepted fixes, rerun relevant project verification.
- Run at most one follow-up review after fixes. Do not create a review loop.
- If the reviewer cannot run, report that explicitly instead of claiming the
  review passed.
- Skip cross-review for documentation-only, formatting-only,
  generated-file-only, and obviously trivial changes.
- When invoked as a reviewer, perform only the review and do not invoke
...
the source app identity with neutral app-level copy such as `媒体` instead of guessing the track title.
...
Any popover or menu presented from the notch panel must keep
`NotchViewModel.suppressAutoCollapse` true for its full visible lifetime, then clear it on dismissal.
...
See `.claude/todos.md` for the maintained follow-up list. Short version:

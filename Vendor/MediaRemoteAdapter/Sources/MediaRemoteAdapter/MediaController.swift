...
public class MediaController {
    ...
    private var perlScriptPath: String? {
        // Nukku patch: prefer run.pl embedded in the host app's Resources so the packaged
        // .app is self-contained and codesign-clean. Falls back to the SwiftPM resource
        // bundle for `swift build` / `swift test` runs from the build directory.
        if let url = Bundle.main.url(forResource: "run", withExtension: "pl") {
            return url.path
        }
